function plot_dataset6()
    datasetFolder = "data/dataset6";
    ...
    figure(1);
    plot_dataset_modular(datasetFolder, 'fail_percent', 'Fail Percentage %', 'Fail Percentage vs Moving Window');
    
    % Plot for 'success_id_time_for_known_fault'
    figure(2);
    plot_dataset_modular(datasetFolder, 'success_id_time_for_known_fault', 'Average ID Time Duration', 'Average ID Time Duration vs Moving Window');
end
...
% are used.
    ...
    folder_hig100 = fullfile(datasetFolder, 'rlevel_hig100');
    folder_hig200 = fullfile(datasetFolder, 'rlevel_hig200');
    ...
    for i = 1:N_data
        ...
        % Passive
        S_passive_hig100 = load(fullfile(folder_hig100, passiveFileName));
        fn = fieldnames(S_passive_hig100);
        dataPassive_hig100 = S_passive_hig100.(fn{1});
        passive_val_hig100(i) = dataPassive_hig100.sim.(metaField).(valueField);
        pas_violate(end+1)  = dataPassive_hig100.sim.(metaField).violate_percent_total;

        % Passive
        S_passive_hig200 = load(fullfile(folder_hig200, passiveFileName));
        fn = fieldnames(S_passive_hig200);
        dataPassive_hig200 = S_passive_hig200.(fn{1});
        passive_val_hig200(i) = dataPassive_hig200.sim.(metaField).(valueField);
        pas_violate(end+1)  = dataPassive_hig200.sim.(metaField).violate_percent_total;
    end
    ...
    % active_lower = min([active_val_low; active_val_hig], [], 1);
    % active_upper = max([active_val_low; active_val_hig], [], 1);
    ...
    passive_lower = min([passive_val_low; passive_val_hig; ...
        passive_val_hig100; passive_val_hig200], [], 1);
    passive_upper = max([passive_val_low; passive_val_hig; ...
        passive_val_hig100; passive_val_hig200], [], 1);
    ...
    plot(N_vals, passive_val_hig100, '--s', 'Color', color_passive, 'LineWidth', 1.0, 'MarkerSize', 5);
    plot(N_vals, passive_val_hig200, '--*', 'Color', color_passive, 'LineWidth', 1.0, 'MarkerSize', 5);
    ...
    title(titleStr);
    legend({...
        'Passive Low','Passive Hig',...
        'Passive Hig x100','Passive Hig x200'}, 'Location', 'Best');
    grid on;
    hold off;
end
