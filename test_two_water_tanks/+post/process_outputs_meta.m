function sim_process = process_outputs_meta(sim, fid)
...
bool_violate_constraints = check_constraints(fid, x1_all, x2_all);
sim_process.outputs_meta.violate_constraints_percent = 100.0*mean(bool_violate_constraints, 2);
sim_process.outputs_meta.violate_percent_total = sum(sim_process.outputs_meta.violate_constraints_percent);

% [update: constraint violation per trials]
sim_process.outputs_meta.violate_trials = [];
for i = 1:sim.config.N_trials
    x1_all = []; x2_all = [];
    data = sim.outputs{i};
    x1_all = [x1_all, data.x_hist(1,:)];
    x2_all = [x2_all, data.x_hist(2,:)];
    bool_violate_constraints = check_constraints(fid, x1_all, x2_all);
    violate_constraints_percent = 100.0*mean(bool_violate_constraints, 2);
    % disp(violate_constraints_percent);
    if(sum(violate_constraints_percent) > 0)
        sim_process.outputs_meta.violate_trials(end+1) = i;
    end
end
